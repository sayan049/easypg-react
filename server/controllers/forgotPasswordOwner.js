...
const jwt = require("jsonwebtoken");
const User = require("../modules/pgProvider"); // This module is for PG Owners/Providers
const { refreshZohoToken } = require("./zohoAuthController"); // Assuming controller path
const ZohoToken = require("../modules/zohoToken"); // Assuming module path
...
const forgotPasswordUser = async (req, res) => {
    ...
    try {
        // --- 1. Initial User Checks (PG Provider/Owner) ---
        const user = await User.findOne({ email });
        if (!user) {
        ...
        // NOTE: Adjusted check based on your code (is_verified_Owner)
        if (!user.is_verified_Owner) { 
            return res.status(403).json({ message: "User account is not verified." });
        }
        if (user.googleId) {
            return res.status(400).json({
                message: "You cannot reset your password because you signed up with Google.",
            });
        }

        // --- 2. Generate Token (KEPT AS IS) ---
        const resetToken = jwt.sign(
            { userId: user._id, email: user.email },
            JWT_SECRET,
            { expiresIn: "1h" }
        ...
        // ❌ REMOVED: Nodemailer transporter creation

        // --- 3. Zoho Token Handling ---
        ...
        // 🎯 4. Dynamic Account ID Retrieval
...
`;
        ...
        // 💡 6. Construct API Payload
        const mailData = {
            fromAddress: `"Messmate" <${USER_EMAIL}>`,
            toAddress: email,
            subject: "Reset Your Messmate Owner Account Password",
            content: htmlResetUrl, // Zoho API uses 'content' for the HTML body
            mailFormat: 'html', 
        };

        // --- 7. Send Email via Zoho API ---
        ...
        console.error("Error in forgotPasswordUser:", errorDetails);
        res.status(500).json({ message: "Server error" });
    }
};
