...
`;
    ...
    try {
        ...
        if (tokenData.isExpired()) {
            ...
            await refreshZohoToken(req, resDummy);
        ...
        // 🔹 3. Database Operations (OTP creation)
        await OTP.deleteMany({ email });
        await OTP.create({ email, code });
        ...
        const mailData = {
            ...
            toAddress: email,
            subject: "OTP Verification - Messmate",
            content: emailHtmlOtp, 
            mailFormat: 'html',
            // Note: Plain text is often handled by the receiving mail client 
            // but for full compliance, Zoho API allows passing the 'text' version too, 
            // though 'content' with 'html' format usually suffices.
        };

        // 🔹 5. Send OTP via Zoho API
        ...
        console.error("Failed to send OTP via Zoho API:", errorDetails);
        res.status(500).json({ error: "Failed to send OTP" });
    }
};
