  ...
  const fetchMessData = async () => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        ...
        toast.error(error.response?.data?.message||"Something went wrong while fetching mess data.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const fetchLikedMesses = async () => {
    try {
      ...
      if (err.response?.status === 404) {
        ...
        toast.error(error.response?.data?.message||"Something went wrong while fetching liked messes.");
      }
    }
  };
  useEffect(() => {
