...
const[loading, setLoading] = useState(false);
  ...
  const handleStatusChange = async (bookingId, status, reason = "") => {
    try {
      ...
      fetchBookings(tab, bookings[tab].page); // Refresh data after status change
      toast.success(`Booking ${status} successfully`);
    } catch (error) {
      console.error(`Status change error (${status}):`, error);
      toast.error(`Failed to ${status} booking`);
      ...
      <div>
        ...
        {loading.list ? (
          <div className="text-center">
            <span className="animate-spin inline-block w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full"></span>
          </div>
        ) : bookings[tab].data.length === 0 ? (
          <EmptyState
            message={`No ${tab} bookings found.`}
            icon={tab === "pending" ? HourglassIcon : XCircleIcon}
          />
        ) : (
          bookings[tab].data.map((booking) => (
            <BookingCard
