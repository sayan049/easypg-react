...
const BookingCard = React.memo(
  ({
    ...
    const isProcessing = loading.action && loading.bookingId === booking._id;
    const isConfirming = isProcessing && loading.actionType === "confirm";
    const isRejecting = isProcessing && loading.actionType === "reject";
  ...
  const handleStatusChange = async (bookingId, status, reason = "") => {
    try {
      setLoading({
        action: true,
        bookingId,
        actionType: status === "confirmed" ? "confirm" : "reject",
        list: false,
        tabChange: false,
        maintenance: false,
      });
