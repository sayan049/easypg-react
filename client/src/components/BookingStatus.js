...
const BookingStatus = ({ bookings, stats, maintenanceRequests }) => {
  const [tab, setTab] = useState("pending");
  const [loading, setLoading] = useState({
    action: false,
    tabChange: false,
  ...
  const BookingCard = ({
    ...
    return (
      <div className="bg-white rounded-xl shadow p-4 w-full md:w-[48%]">
        <h3 className="text-lg font-semibold mb-2">
          {booking.room} – {booking.bedsBooked} bed(s)
        </h3>
        <p className="text-sm text-gray-600 mb-2">
          Check-in: {new Date(booking.period.startDate).toLocaleDateString()}<br />
          Check-out: {new Date(booking.period.endDate).toLocaleDateString()}
        </p>
        <p className="text-sm text-gray-500 mb-2">Status: {booking.status}</p>
        {booking.status === "pending" && (
        ...
        {/* Optionally show maintenance info for confirmed bookings */}
        {booking.status === "confirmed" && maintenanceRequests?.requests?.length > 0 && (
          <div className="mt-4">
            <p className="text-sm font-medium text-gray-700 mb-2">Maintenance Requests:</p>
            <ul className="space-y-2">
              {maintenanceRequests.requests
                .filter((r) => r.bookingId === booking._id)
                .map((req) => (
                  <li
                    key={req._id}
                  ...
                  >
                    ...
                    {req.status === "pending" && (
                      <div className="flex gap-1">
                        <button
                          onClick={() => handleMaintenanceResolve(req._id, "Resolved")}
                          className="text-xs bg-green-500 text-white px-2 py-1 rounded"
                        >
                          Resolve
                        </button>
                        <button
                          onClick={() => handleMaintenanceCancel(req._id, "Cancelled")}
        ...
        <div className="flex justify-between items-center border-b pb-2">
          <div className="flex gap-4">
            {["pending", "confirmed", "rejected"].map((status) => (
              ...
              >
                {status.charAt(0).toUpperCase() + status.slice(1)} Bookings ({bookings[status].total})
              </button>
            ))}
          </div>
        </div>
          ...
          {bookings[tab].data.length > 0 ? (
            bookings[tab].data.map((booking) => (
              <BookingCard
                ...
                loading={loading.action}
                maintenanceRequests={maintenanceRequests}
                handleMaintenanceCancel={(id, msg) =>
                  handleMaintenanceStatusChange(id, "cancelled", msg)
                }
                handleMaintenanceResolve={(id, msg) =>
                  handleMaintenanceStatusChange(id, "resolved", msg)
                }
              />
            ))
          ) : (
            <p className="text-gray-500">No {tab} bookings found.</p>
          )}
        </div>
      </div>
    </div>
...
BookingStatus.propTypes = {
  ...
  maintenanceRequests: PropTypes.object.isRequired,
};
