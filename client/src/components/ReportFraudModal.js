...
function ReportFraudModal({ stay, onClose, onSubmit, submitting, success }) {
  ...
  const handleSubmit = () => {
    // Validate that either a predefined reason is selected or a description is entered
    ...
    // Combine reason and description in a single string to submit
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg max-w-md w-full p-6 relative">
        ...
        <h2 className="text-xl font-semibold mb-4">Report Fraud</h2>

        <p className="mb-3">
          Reporting for: <strong>{stay.pgOwner?.messName || "this PG"}</strong>
        </p>

        <label className="block mb-2 font-medium text-gray-700" htmlFor="reason-select">
          Select a reason:
        </label>
        <select
          id="reason-select"
        ...
        <label className="block mb-2 font-medium text-gray-700" htmlFor="description-textarea">
          Additional details (optional):
        </label>
        <textarea
          id="description-textarea"
          rows={4}
          className={`w-full border rounded p-2 mb-1 ${error ? "border-red-500" : "border-gray-300"}`}
