      ...
      <div className={`grid gap-4 p-2 sm:p-4 ${isChecked ? "grid-cols-1" : ""}`}>
        {messData.map((owner, index) => (
          <div
            key={owner._id}
            ref={index === messData.length - 1 ? lastCardRef : null}
            className={`relative flip-card mb-4 w-full ${
              isChecked ? "md:min-h-[16rem]" : "md:min-h-[14rem]"
            } ${
              selected === owner._id && isChecked
                ? "ring-2 ring-blue-500"
                : "ring-1 ring-gray-200"
          ...
          >
            ...
            >
              ...
              >
                {!isChecked && (
                  <div className="relative h-48 md:h-full md:max-h-[14rem] rounded-lg overflow-hidden">
                    <img
                      loading="lazy"
                      src={owner.profilePhoto}
                      alt="Mess"
