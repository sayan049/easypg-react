...
function MessBars({ isChecked, checkFeatures, userLocation, coords, setPgCount }) {
  ...
  return (
    <>
      <style>{styles}</style>
      <div className={`grid gap-4 p-2 sm:p-4 ${isChecked ? "md:grid-cols-2" : ""}`}>
        {messData.map((owner) => (
          <div
            key={owner._id}
            className={`relative flip-card h-[31rem] mb-4 ${isChecked ? "md:h-[19rem]" : "md:h-[15rem]"} ${
          ...
          >
            <div
              className={`flip-card-inner ${
                flipped[owner._id] && window.innerWidth < 768 ? "flipped" : ""
              } ${isChecked && window.innerWidth >= 768 ? "flex" : ""}`}
            >
              {/* FRONT SIDE */}
              <div
                className={`flip-card-front ${
                  isChecked && window.innerWidth >= 768
                    ? "hidden"
                    : "grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-4"
                }`}
              >
                {!isChecked && (
                  <div className="relative h-48 md:h-full rounded-lg overflow-hidden">
              ...
              <div
                className={`flip-card-back ${
                  isChecked && window.innerWidth >= 768
                    ? "static transform-none relative w-full h-full rounded-xl"
                    : ""
