...
// import { findMessUrl } from "../constant/urls";
// import {
//   FaWind, FaTv, FaBatteryFull, FaWifi, FaUtensils, FaTint, FaBed, FaFemale, FaMale, FaUsers,
// } from "react-icons/fa";

// function MessBars({
//   isChecked, checkFeatures, userLocation, coords, setPgCount, finalGender,
// }) {
//   const [messData, setMessData] = useState([]);
//   const [visibleMessBars, setVisibleMessBars] = useState([]);
...
//     if (Array.isArray(location) && location.length === 2) {
//       const [lng, lat] = location;
//       if (typeof coords === "function") coords({ lat, lng });
//     }
//   };
...
//         const res = await axios.get(findMessUrl, {
//           params: { lat: parseFloat(userLocation.lat), lng: parseFloat(userLocation.lng) },
//         });

//         const filteredData = (Array.isArray(res.data) ? res.data : []).filter((owner) => {
//           const facilitiesArray = Array.isArray(owner.facility)
...
//         setMessData(filteredData);
//         setVisibleMessBars(filteredData.slice(0, ITEMS_PER_PAGE));
//         setPgCount(filteredData.length);
...
//     setVisibleMessBars((prev) => [...prev, ...messData.slice(start, end)]);
//     setPage(nextPage);
//   };
...
//         {visibleMessBars.map((owner) => (
//           <div
//             key={owner._id}
//             className={`relative flip-card mb-4 h-[35rem] md:h-[16rem] w-full
//               ${selected === owner._id && isChecked ? "ring-2 ring-blue-500" : "ring-1 ring-gray-200"}
//               bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow p-4`}
//             onClick={() => owner?.location?.coordinates && clickCords(owner.location.coordinates, owner._id)}
//           >
//             <div className={`flip-card-inner ${flipped[owner._id] ? "flipped" : ""}`}>
//               {/* FRONT SIDE */}
//               <div className={`flip-card-front grid gap-4 w-full h-full ${isChecked ? "grid-cols-1" : "md:grid-cols-[1fr_2fr]"}`}>
//                 {!isChecked && (
//                   <div className="relative h-48 md:h-full rounded-lg overflow-hidden">
//                     <img loading="lazy" src={owner.profilePhoto} alt="Mess" className="w-full h-full object-cover" />
//                   </div>
//                 )}
//                 <div className="flex flex-col justify-between h-full">
//                   <div>
//                     <div className="flex justify-between items-start">
//                       <h3 className="text-xl font-semibold text-gray-900">{owner.messName}</h3>
//                       <button className="p-2 hover:bg-blue-200 rounded-full transition-colors md:hidden"
//                         onClick={(e) => { e.stopPropagation(); toggleFlip(owner._id); }}>
